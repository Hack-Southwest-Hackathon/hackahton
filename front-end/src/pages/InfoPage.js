    ...
    },
{
  ...
  return (
    <div>
        <h1>{infoTabs[currentTabIndex].title}</h1>
        <p>{infoTabs[currentTabIndex].text}</p>
        <div>
            {!currentTabIndex==0 && <button onClick={()=>handleTabClick(-1)}>Back</button>}
            {currentTabIndex==(infoTabs.length-1) ? <></> : <button onClick={()=>handleTabClick(1)}>Next</button>}
        </div>
    </div>
  )
}
